...
public class Songs {
    ...
    public static void main(String[] args) {
        Stream<Song> str = getSongs().stream();

        List<Song> song =  str.filter(m->m.getGenre().equals("Rock"))
                .collect(Collectors.toList());
        System.out.println(song);

        List<String> songs =  getSongs().stream().filter(m->m.getGenre().equals("Rock"))
                .map(m->m.getTitle())
                .collect(Collectors.toList());
        System.out.println(songs);

        List<Song> sg = getSongs().stream().sorted((y1,y2)->y2.getYear() - y1.getYear())
                .collect(Collectors.toList());
